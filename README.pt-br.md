...
1. Abra o Protégé
2. `File → Open` → selecione `ontocup.ttl` (requer internet para resolver imports) **ou** `ontocup-standalone.ttl` (sem dependências externas)

### Usar com Ontop (SPARQL over relational DB)

Recomenda-se `ontocup-standalone.ttl` para evitar problemas de resolução de imports em ambientes Ontop. Configure o arquivo como a ontologia base no mapping do Ontop.
